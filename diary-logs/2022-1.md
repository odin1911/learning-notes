...
引入tailwind没有样式，检查一下，不行就先不用

建了个干净的项目可以并存，还要对比下项目代码
...
标注项目重新配置了一下，可以显示tailwind的样式了，也没找到错误原因

## 2022-03-10

标注开会，只做documents和标注页面相关ui
...
看了tailwind样式表，有些样式都不了解，清理也没有测试（大概了解了），先拿部分用起来

目前优先保持进度，把2个主要界面搭出来

## 2022-03-15

接口给到了，把api层写掉，要保留query层，接口不要用带collections的那组

使用不带collections的接口，由于server特殊处理了collectionId，以后可以要改代码，注意一下

## 2022-03-16

在家办公

处理接口请求，缓存机制，目前接口数据不对，还是要用模拟数据，考虑引入msw

Nice Modal

测试documents的数据请求和排序过滤操作，已经引入msw，写模拟数据
...
论文标注系统开发。需求移交，高保移交，后端接口讨论，论文列表开发20%
